...
**Goal:** Add a `hereticMOE language-map` diagnostic that measures every aligned prompt once, caches per-layer BF16-model residual geometry, and evaluates language/prompt subsets without another model invocation.
...
### Task 4: `hereticMOE language-map` CLI and CPU integration test
...
**Interfaces:**
- Adds `hereticMOE language-map capture ...`.
- Adds `hereticMOE language-map analyze --cache-dir ...`.
- Adds `hereticMOE language-map run ...` as capture followed by analyze.

- [ ] **Step 1: Write failing CLI dispatch and dry-run tests**

```python
def test_cli_dispatches_language_map_without_supervisor(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["hereticMOE", "language-map", "--help"])
...
**Files:**
- Create outside git: `F:/AI/hf_originals/heretic_out/research/results/language_geometry_v1/gemma4_e4b/`
...
Run `hereticMOE language-map run --dry-run` with the six complete train files
and `--expected-languages en,ru,es --expected-per-cell 1200`.

Expected: PASS, 7,200 rows, exact coverage, no GPU allocation.

- [ ] **Step 2: Run a 20-row-per-cell Gemma smoke**
...
final cache/report hashes. Re-run `language-map analyze` from cache and confirm
